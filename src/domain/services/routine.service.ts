...
export class RoutineServiceImpl implements RoutineService {
  ...
  public async addRoutine({ userId, routine, secret }: AddRoutineInput): Promise<AddRoutineOutput> {
    //1. user가 admin인지 검사
    ...
    //3. routine name 중복검사
    const result = await this.routineRepository.findOneByRoutineName(routine.name);
  ...
  public async getAllRoutines({nextCursor}: GetAllRoutinesInput): Promise<GetAllRoutinesOutput> {
  ...
  public async getRoutineDetail({ routineId }: GetRoutineDetailInput): Promise<GetRoutineDetailOutput> {
    let routine: Routine;
...
-> 0원 or XXXXX원 결제 (0원일 경우 유저에게는 결제 과정은 생략)
     */
  public async buyRoutine({ userId, routineId }: BuyRoutineInput): Promise<void> {
    //TODO 유료인 경우
    //TODO 무료인 경우
