...
export abstract class ImageProvider {
   ...
   *  */
  abstract requestImageToCDN(
    mappedImageModel: ImageModel,
  ): Promise<string | string[]>;

  /**
   * db에서 얻은 이미지객체를 매개변수로써 ImageModel로 매핑한후 반환
   */
  abstract getMappedImageModel(imageDocument: ImageModel): ImageModel;

  /**
   * cloud에 origin 이미지 저장한 후 데이터가 저장된 url key를 반환합니다
   */
  abstract putImageFileToCloudDb(
   ...
   */
  abstract deleteImageFileFromCloudDb(cloudKey: string): void;

  /**
   * imageRepository에 이미지객체를 저장하기 위한 createImageDto생성
   */
  abstract mapCreateImageDtoByCloudKey(
    cloudKeys: string[],
    type: ImageType,
