...
export class AddRoutineResponseDto {
  ...
  @ApiProperty({ description: '루틴 소개 스크립트', example: '요가파이어' })
  introductionScript: string;

  @ApiProperty({ description: '동기부여 문장', example: '뻣뻣한 몸에 부드러움을 선사합시다' })
  motivation: string;

  @ApiProperty({ description: '가격', example: 0 })
  price: number;

  //product id
  @ApiProperty({ description: '관련 상품id 들', example: ['id가 올 예정'] })
