...
export class AuthServiceImpl implements AuthService {
  ...
  private async _signInWithGoogleAccessToken({
    ...
    const client = this._googleAuthProvider.getGoogleClient(process.env.GOOGLE_CLIENT);
  ...
  public async reissueAccessToken({
    ...
    const result: boolean = await this._hashProvider.compare(refreshToken, user['refresh_token']);
