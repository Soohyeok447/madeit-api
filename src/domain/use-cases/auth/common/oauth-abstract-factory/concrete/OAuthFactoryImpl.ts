...
export class OAuthFactoryImpl implements OAuthFactory {
  ...
  createOAuth(thirdPartyAccessToken: string, provider: Provider): OAuth {
    switch (provider) {
      case Provider.google: {
        return new GoogleOAuth(thirdPartyAccessToken, this._googleAuthProvider);
      }
      case Provider.kakao: {
        return new KakaoOAuth(thirdPartyAccessToken, this._httpClient);
      }
      default:
