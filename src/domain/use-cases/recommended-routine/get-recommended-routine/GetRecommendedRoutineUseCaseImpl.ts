...
@Injectable()
export class GetRecommendedRoutineUseCaseImpl implements GetRecommendedRoutineUseCase {
  constructor(
  ...
  public async execute({
    ...
    const routine: RecommendedRoutineModel = await this._recommendRoutineRepository.findOne(recommendedRoutineId);

    CommonRecommendedRoutineService.assertRoutineExistence(routine);

    const output: GetRecommendedRoutineResponseDto = await this._mapModelToResponseDto(routine)

    return output;
  }


  private async _mapModelToResponseDto(routine: RecommendedRoutineModel): Promise<GetRecommendedRoutineResponseDto> {
    let thumbnailUrl;
