...
import { GetRecommendedRoutinesResponseDto, RecommendedRoutineItems } from "./dtos/GetRecommendedRoutinesResponseDto";
...
@Injectable()
export class GetRecommendedRoutinesUseCaseImpl implements GetRecommendedRoutinesUseCase {
  constructor(
  ...
  public async execute({
    ...
    const recommendedRoutines: RecommendedRoutineModel[] | [] = await this._recommendRoutineRepository.findAll(size, next);

    const output: GetRecommendedRoutinesResponseDto = await this._mapModelToResponseDto(recommendedRoutines, size, next);

    return output;
  }

  private async _mapModelToResponseDto(recommendedRoutines: RecommendedRoutineModel[] | [], size: number, next: string): Promise<GetRecommendedRoutinesResponseDto> {
    ...
    const nextCursor = hasMore ? recommendedRoutines[recommendedRoutines.length - 1]['_id'] : null;
