...
import { ExchangePointRequestDto } from '../../../src/adapter/exchange/exchange-point/ExchangePointRequestDto';

describe('exchangePoint e2e test', () => {
  ...
  afterAll(async () => {
    ...
    await dbConnection.collection('exchange-orders').deleteMany({});
  ...
  describe('POST v1/exchange before issue exchangeToken', () => {
    describe('Request exchange', () => {
      it('errorCode 81 should be thrown', async () => {
        const dto: ExchangePointRequestDto = {
          bank: '테스트은행',
        ...
        expect(res.body.errorCode).toEqual(81);
      });
    });
  });
    ...
    describe('Request exchangeToken', () => {
      describe('with correct serial', () => {
        it('exchangeToken should be return', async () => {
          ...
          exchangeToken = res.body.exchangeToken;
  ...
  describe('POST v1/exchange after issue exchangeToken', () => {
    describe('Request exchange', () => {
      describe('with not enough point', () => {
        it('errorCode 5 should be thrown', async () => {
          const dto: ExchangePointRequestDto = {
            bank: '테스트은행',
            account: '000000000000',
            amount: 100,
          ...
          const res: request.Response = await request(httpServer)
            .post('/v1/exchange')
            .set('Authorization', `Bearer ${exchangeToken}`)
            .set('Accept', 'application/json')
            .type('application/json')
            .send(dto);

          expect(res.statusCode).toEqual(404);
          expect(res.body.errorCode).toEqual(5);
        });
      });

      describe('with enough point', () => {
        it('{} should be return', async () => {
          const dto: ExchangePointRequestDto = {
            bank: '테스트은행',
            account: '000000000000',
